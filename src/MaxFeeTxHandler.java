...
public class MaxFeeTxHandler {
    ...
    // Extra Credit:Create a second file called MaxFeeTxHandler.java with similar API
    // whose handleTxs finds a set of transactions with maximum total transaction fees
    // (i.e. maximize the sum over all transactions in the set of
    // (sum of input values - sum of output values)).
    // Finding such a set of transactions is NP hard and thus find only a heuristic which might
    // increase the total transaction fees.
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        // Sort transactions by fee
        List<Transaction> sortedTxs = new ArrayList<Transaction>(Arrays.asList(possibleTxs));
        sortedTxs.sort((tx1, tx2) -> {
            double fee1 = calculateTransactionFees(tx1);
            double fee2 = calculateTransactionFees(tx2);
        ...
        for (int i = 0; i < sortedTxs.size(); i++) {
            if(isValidTx(sortedTxs.get(i))){
                validTxs.add(sortedTxs.get(i));

                List<Transaction.Output> outputs = sortedTxs.get(i).getOutputs();
                for(int j = 0; j < outputs.size(); j++){
                    UTXO utxo = new UTXO(sortedTxs.get(i).getHash(), j);
                    utxoPool.addUTXO(utxo, outputs.get(j));
                }

                List<Transaction.Input> inputs = sortedTxs.get(i).getInputs();
